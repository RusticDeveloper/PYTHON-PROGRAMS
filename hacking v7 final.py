...
def display_password_list(window, location):
    ...
    for password in password_list:
        embed=embed_password(password,20)
        display_line(window, embed, location)
...
def get_guesses(window, password, location, attempts_left):
    ...
    hint_y = window.get_font_height()
    guess = prompt_user(window, prompt, location)
    attempts_left = attempts_left - 1
    hint_location=[hint_x,hint_y]
...
def embed_password(password, size):
    ...
    last_split_index=password_size+split_index
    for index in range (0,split_index):
        embedding=embedding+choice(fill)
    embedding+=password
    for index in range(last_split_index,size):
        embedding=embedding+choice(fill)
    return embedding
...
def display_hint(window, password, guess, location):
    ...
    for index in range(0,len(password)):
        if (index<len(guess)):
            ...
            continue
    hint_outcome=[password+" INCORRECT",str(rigth_letters)+"/7 IN MATCHING POSITIONS"]
    display_line(window, hint_outcome[0],location)        
    display_line(window,hint_outcome[1] ,location)
